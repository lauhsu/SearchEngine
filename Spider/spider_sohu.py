...
def GetNews():
    while len(url_set) != 0:
        try:
            ...
            html = urllib.urlopen(article.url).read().decode('utf8')
            soup = bs4.BeautifulSoup(html, 'html.parser')
            article.title = soup.find('title').get_text()  # 标题信息
            keywords='keywords'
            res0 = re.compile(keywords)
            if soup.find('meta', {'name': res0}).__getitem__('name') == "keywords":
                article.keywords = soup.find('meta', {'name': res0}).__getitem__('content')
            else:
                article.keywords = ""
            author = 'mediaid'
            res = re.compile(author)
            if soup.find('meta', {'name': res}).__getitem__('name') == "mediaid":
                article.author = soup.find('meta', {'name': res}).__getitem__('content')
            else:
                article.author = ""
            published_time= 'og:release_date'
            res1 = re.compile(published_time)
            if soup.find('meta', {'property': res1}).__getitem__('property') == "og:release_date":
