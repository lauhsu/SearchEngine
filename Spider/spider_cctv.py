...
def GetNews():
    while len(url_set) != 0:
        try:
            ...
            if soup.find('meta', {'name': res1}).__getitem__('name') == "description":
                article.content = soup.find('meta', {'name': res1}).__getitem__('content')  # 作者
            else:
                article.content = ""
            SaveNews(article)
