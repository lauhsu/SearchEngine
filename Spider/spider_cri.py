...
def GetNews():
    while len(url_set) != 0:
        try:
            ...
            article.author = author = soup.find('span', {'class': 'asource'}).get_text()  # 作者


            article.date = soup.find('span', {'class': 'acreatedtime'}).get_text() # 作者

            article.content = soup.find('div', {'class': 'abody'}).get_text()
            SaveNews(article)
