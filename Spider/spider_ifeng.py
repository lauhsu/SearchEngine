...
def GetNews():
    while len(url_set) != 0:
        try:
            ...
            if soup.find('meta', {'name': res}).__getitem__('name') == "og:category":
                article.author = soup.find('meta', {'name': res}).__getitem__('content')  # 作者
            else:
                article.author = ""
            published_time = 'og:time'
            res1 = re.compile(published_time)
            if soup.find('meta', {'name': res1}).__getitem__('name') == "og:time":
                article.date = soup.find('meta', {'name': res1}).__getitem__('content')  # 作者
            else:
                article.date = ""
            content= soup.select('.js_selection_area')
