...
def GetNews():
    while len(url_set) != 0:
        try:
            ...
            if soup.find('meta', {'name': res}).__getitem__('name') == "source":
            ...
            if soup.find('meta', {'name': res1}).__getitem__('name') == "publishdate":
                ...
                article.date = ""
            article.content = soup.find('div', {'class': 'newscontent'}).get_text()
            SaveNews(article)
