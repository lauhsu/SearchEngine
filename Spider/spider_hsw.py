...
def GetNews():
    while len(url_set) != 0:
        try:
            ...
            article.author = soup.find('div', {'class': 'origin'}).get_text()    # 作者


            article.date = soup.find('span', {'class': 'article-time'}).get_text()   # 作者

            article.content = soup.find('div', {'class': 'contentBox'}).get_text()
            SaveNews(article)
